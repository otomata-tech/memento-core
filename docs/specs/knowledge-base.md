...
> doctrine. **Open Kairos / Kairos AI = workspace #1** (cf. memory
...
- **Multi-workspace = multi-tenant.** Un `MemWorkspace` = un projet/domaine de
  ...
  `mem_memberships` — cf. `docs/access-control.md`). Open Kairos est le premier.
...
// ── Workspace : le tenant. Un domaine de savoir isolé (Open Kairos = #1) ────
model MemWorkspace {
...
de données*, pas un couplage. Premier workspace visé : **Open Kairos** (docs Arnaud :
...
**Ouvertes** :
- **Taxonomie initiale des sections** (workspace Open Kairos) = décision **Arnaud/Cyril**,
